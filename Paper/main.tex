...
A CCD camera (340M Fast Frame, Thorlabs) was included in the optomechanical system (OTKB(/M)\DS{is there an additional parenthesis? } Modular Optical Tweezers, Thorlabs). It process the input LED light after crossing the chip and being reflected by a first mirror, placed for structural reasons, and a second one which has the splitting light task. \DS{A camera doesn't do light processing. Secondly, I think you can make this sentence shorter to make the concept clearer}. The CCD camera was connected through a USB connection with a PC for the data acquisition phase and then the analysis one in the dedicated software platforms.\DS{The text is a little bit contorted. It seems you've the tendency to use "one" to avoid rewriting the same word, which is good. However, sometimes, this is not always helpful during the reading phase.} 
...
\end{figure}
A magnification of 10X (PLN, Olympus) has been considered. \DS{"A magnification of 10X has been used to..."} 
...
The fluids employed for the experiments were obtained by diluting micro-particles in different solutions. The micro-particles were of two types: live cells of eukaryotic origins with a diameter of 5~$[\rm~\mu m]$ and artificial silica beads with a diameter of 6~$[\rm~\mu m]$ \DS{we can explain why we've used two types of particles.} Four different types of living cell conditions have been examined. In particular viable cells, 2 days overgrowth, 5 days overgrowth and exposed to cell death stimuli with \DS{water ($H_2O_2$)}. \DS{Did we say to mention living and death cells. Didn't we?} 
...
\DS{You can avoid using \textbackslash\textbackslash for the line return. Sometimes doesn't create anything, sometimes it creates undesired effects in the text formatting.}
